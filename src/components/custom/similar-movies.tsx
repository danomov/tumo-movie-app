      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {similar.slice(0, 6).map((similarMovie: IMovie) => (
          <Link key={similarMovie.id} to={constructMoviePath(similarMovie.id)}>
            <Card className="cursor-pointer transition-all hover:scale-105 hover:shadow-lg p-0 h-full overflow-hidden">
              <CardContent className="p-0">
                <div className="relative aspect-[3/4]">
                  <img
                    ...
                    src={constructPosterPath(similarMovie.poster_path)}
                  />
                </div>
                <div className="p-3">
